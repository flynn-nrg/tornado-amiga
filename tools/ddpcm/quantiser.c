...
int16_t predict(int16_t y1, int16_t y2) {
  ...
  if(p > INT16_MAX) return INT16_MAX / 2;
  if(p < INT16_MIN) return INT16_MIN / 2;
  return (int16_t) p;
}
...
// Takes the array of sorted deltas and outputs a 64 entry array of quantised values.
// Each value is the wieghted sum of all the deltas in that bucket.
static int16_t *quantise16Weighted(q_entry **sortedDeltas) {
	...
	for(uint32_t i = 0; i < 64; i++) {
		...
		if(numSamples > 0.0) {
			for(uint32_t j = 0; j < 1024; j++) {
				weighted += (((float) sortedDeltas[i][j].value) * ((float) sortedDeltas[i][j].score)) / numSamples;
			}
...
// Takes the arrays of sorted deltas and outputs a 64 entry array of quantised values.
// The more often a value is present the more weight it has on the quatised entry.
