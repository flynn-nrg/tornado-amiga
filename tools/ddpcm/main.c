...
static void usage(char *progname) {
  ...
  fprintf(stderr, "Usage: %s -q -d <delta-computation> -i <input_file> -o <output_file> -p <preview_file>\n", progname);
  exit(EXIT_FAILURE);
}
...
int main(int argc, char **argv) {
	...
	uint32_t fpqt = framesPerQtable(numFrames, DDPCM_MAX_TABLES, DDPCM_MIN_TABLES);
	...
	for(uint32_t i = 0; i < ddpcmh->numQTables; i++) {
		ddpcmh->qtablesLeft[i] = quantiser(&leftSrc[lOffset], samplesPerTable, deltaMode);
		for(uint32_t j = 0; j < ddpcmh->framesPerQTable; j++) {
			ddpcmh->scalesLeft[lFrame++] = encodeFrame(&leftSrc[lOffset], &lDest[lDstOffset], ddpcmh->qtablesLeft[i]);
	...
	uint32_t rFrame = 0;
	for(uint32_t i = 0; i < ddpcmh->numQTables; i++) {
		ddpcmh->qtablesRight[i] = quantiser(&rightSrc[rOffset], samplesPerTable, deltaMode);
		for(uint32_t j = 0; j < ddpcmh->framesPerQTable; j++) {
			ddpcmh->scalesRight[rFrame++] = encodeFrame(&rightSrc[rOffset], &rDest[rDstOffset], ddpcmh->qtablesRight[i]);
	...
	printf("Compressed size: %u bytes (%u bytes in qtables, %u bytes in scale tables, %u bytes in compressed frames)\n", qtableUsage + framesUsage + scalesUsage, qtableUsage, scalesUsage, framesUsage);
	...
	if(save_preview) {
		...
		for(uint32_t i = 0; i < ddpcmh->numQTables; i++) {
			for(uint32_t j = 0; j < ddpcmh->framesPerQTable; j++) {
				decodeFrame(&uleftSrc[ulOffset], &ulDest[ulDstOffset], ddpcmh->qtablesLeft[i], ddpcmh->scalesLeft[ulFrame++]);
				ulOffset += DDPCM_COMPRESSED_FRAME_SIZE;
				ulDstOffset += DDPCM_FRAME_NUMSAMPLES;
				printf("Decompressing left channel: %i/%i\r", ulDone++, ddpcmh->numFrames);
		...
		for(uint32_t i = 0; i < ddpcmh->numQTables; i++) {
			for(uint32_t j = 0; j < ddpcmh->framesPerQTable; j++) {
				decodeFrame(&urightSrc[urOffset], &urDest[urDstOffset], ddpcmh->qtablesRight[i], ddpcmh->scalesRight[urFrame++]);
				urOffset += DDPCM_COMPRESSED_FRAME_SIZE;
				urDstOffset += DDPCM_FRAME_NUMSAMPLES;
				printf("Decompressing right channel: %i/%i\r", urDone++, ddpcmh->numFrames);
	...
	fwrite(ddpcmh->right, ddpcmh->numFrames, DDPCM_COMPRESSED_FRAME_SIZE, outfile);
